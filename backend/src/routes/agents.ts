...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    const oktaAgent = await okta.createAgent(name, description || '');
    const [agent] = await db
      ...
      .values({
        ...
        status: oktaAgent.status === 'ACTIVE' ? 'active' : 'pending',
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ ...agent, resources: linked.map((l) => l.resource) });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
});
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (agent.oktaAgentId) {
      await okta.deleteAgent(agent.oktaAgentId).catch(() => {});
    }
    await db.delete(agentResources).where(eq(agentResources.agentId, req.params.id));
